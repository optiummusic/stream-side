...
impl VideoBackend for DesktopFfmpegBackend {
    fn push_encoded(&mut self, payload: &[u8], _frame_id: u64) -> Result<(), BackendError> {
        // Создаём ffmpeg::Packet и копируем в него данные.
        // Копия неизбежна: ffmpeg владеет своей памятью.
